 ...
 * A ImportHelper manages the one-time import of old .covidtracker files
 *
...
import org.techsfaction.covidtracker.core.WayPoint
...
object ImportHelper {
    ...
    /* Converts older tracks of type .covidtracker into the new format  */
    fun convertOldTracks(context: Context) {
        ...
        if (trackFolder != null && trackFolder.exists() && trackFolder.isDirectory) {
            trackFolder.listFiles()?.forEach { file ->
                if (file.name.endsWith(".covidtracker")) {
